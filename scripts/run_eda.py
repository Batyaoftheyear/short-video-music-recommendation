...
def run() -> None:
    ...
    # Build summary text
    video_total = len(video_meta)
    audio_total = len(audio_meta)
    imbalance_flag = bool((balance_df["video_count"] == 0).any() or (balance_df["audio_count"] == 0).any())
    imbalance_flag = imbalance_flag or bool(
        (balance_df[["video_count", "audio_count"]].max(axis=1) - balance_df[["video_count", "audio_count"]].min(axis=1)).max() > 0.25 * max(video_total, audio_total, 1)
    )
    video_feat_missing_total = int(video_feat_missing_df["missing_count"].sum()) if not video_feat_missing_df.empty else 0
    audio_feat_missing_total = int(audio_feat_missing_df["missing_count"].sum()) if not audio_feat_missing_df.empty else 0
    missing_files_total = len(video_meta_missing) + len(audio_meta_missing)
    duplicates_total = len(duplicates_df)

    anomaly_notes = []
    if not video_anomalies.empty:
        anomaly_notes.append(f"video anomalies: {len(video_anomalies)}")
    if not audio_anomalies.empty:
        anomaly_notes.append(f"audio anomalies: {len(audio_anomalies)}")
    if not anomaly_notes:
        anomaly_notes.append("no anomalies detected by current rules")

    summary_lines = [
        "EDA SUMMARY",
        f"Video metadata source: {video_meta_source}",
        f"Audio metadata source: {audio_meta_source}",
        f"Video objects in metadata: {video_total}",
        f"Audio objects in metadata: {audio_total}",
        "Video counts by mood:",
    ]
    for _, row in video_counts.iterrows():
        summary_lines.append(f"- {row['mood']}: {int(row['video_count'])}")
    summary_lines.append("Audio counts by mood:")
    for _, row in audio_counts.iterrows():
        summary_lines.append(f"- {row['mood']}: {int(row['audio_count'])}")
    summary_lines.extend(
        [
            f"Category imbalance detected: {'yes' if imbalance_flag else 'no'}",
            f"Feature missing values (video key features): {video_feat_missing_total}",
            f"Feature missing values (audio key features): {audio_feat_missing_total}",
            f"Missing files from metadata on disk: {missing_files_total}",
            f"Duplicate records detected: {duplicates_total}",
            f"Mood mismatches metadata vs features (video/audio): {video_mood_mismatch}/{audio_mood_mismatch}",
            f"Feature row cardinality issues per file (video/audio): {video_feat_per_file_issues}/{audio_feat_per_file_issues}",
            f"Empty moods in video: {', '.join(empty_video_moods) if empty_video_moods else 'none'}",
            f"Empty moods in audio: {', '.join(empty_audio_moods) if empty_audio_moods else 'none'}",
            f"Anomalies: {', '.join(anomaly_notes)}",
            "Conclusions for stage 1:",
            "- Dataset consistency checks are completed for metadata/features and file-system alignment.",
            "- Extracted features are available for retrieval/ranking experiments without model training.",
            "- Reported anomalies and missingness should be filtered or handled before stage 2 ranking pipeline.",
        ]
    )
    summary_text = "\n".join(summary_lines)
    ...
    # Draft report section (ru)
    draft_lines = [
        "# Черновик раздела EDA (этап 1)",
        "",
        "## Что было проанализировано",
        f"- Использованы актуальные метаданные: `{video_meta_source.name}` для видео и `{audio_meta_source.name}` для аудио.",
        "- Выполнены проверки консистентности между metadata, features и фактическими файлами на диске.",
        "- Построены распределения ключевых признаков и сравнение баланса категорий между видео и аудио.",
        "",
        "## Основные наблюдения",
        f"- Количество видеообъектов: **{video_total}**, количество аудиообъектов: **{audio_total}**.",
        "- Распределения длительностей, динамики (flow), яркости/насыщенности (видео) и bpm/энергии/спектральных признаков (аудио) сохранены в `reports/figures/`.",
        f"- Выявленные аномалии: видео — **{len(video_anomalies)}**, аудио — **{len(audio_anomalies)}**.",
        "",
        "## Ограничения и проблемы",
        f"- Missing files по metadata: **{missing_files_total}**.",
        f"- Дубликаты записей: **{duplicates_total}**.",
        f"- Пропуски в ключевых признаках: видео — **{video_feat_missing_total}**, аудио — **{audio_feat_missing_total}**.",
        f"- Несовпадения `mood` между metadata и features: видео — **{video_mood_mismatch}**, аудио — **{audio_mood_mismatch}**.",
        "",
        "## Выводы для следующего этапа",
        "- Датасет и признаки достаточно структурированы для перехода к этапу retrieval/ranking.",
        "- Перед ранжированием рекомендуется исключить или отдельно обработать записи из таблиц `video_anomalies.csv` и `audio_anomalies.csv`.",
        "- Для устойчивости экспериментов следует учитывать дисбаланс категорий и пропуски признаков при подготовке train/val сплитов и метрик оценки.",
    ]
    (root / "reports" / "eda_section_draft.md").write_text("\n".join(draft_lines), encoding="utf-8")
